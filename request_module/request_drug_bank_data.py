...
class DrugInteraction:
    def __init__(self, drug_name, interaction_description):
    ...
    def __repr__(self):
        return f"DrugInteraction(drug_name={self.drug_name}, interaction_description={self.interaction_description})"
...
class DrugInteractionResponse:
    ...
    def parse_interaction(self, entry):
        try:
            if '>' in entry[0] and '<' in entry[0]:
                drug_name = entry[0].split('">')[1].split('<')[0]
            else:
                drug_name = entry[0]
        except IndexError:
            drug_name = "Unknown"
        ...
        return drug_name, interaction_description
    ...
    def print_interactions(self):
        ...
        for interaction in self.data:
            interactions.append((interaction.drug_name, interaction.interaction_description))
        return interactions
...
def call_api():
    ...
    header = {
        ...
        # Placeholder for the actual cookie
    }
    ...
    for index, entry in enumerate(entries):
        ...
        for page in range(number_of_pages):
            ...
            for drug_name, interaction_description in interactions:
